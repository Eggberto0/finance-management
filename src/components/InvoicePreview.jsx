...
export default function InvoicePreview({ invoicePreview, accounts = [] }) {
                    ...
                    return (
                        <div key={card.id} className="flex flex-col gap-3">
                            <div className="flex items-center gap-2">
                                ...
                                <p className="text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide">{card.name}</p>
                            </div>
                                        ...
                                        <div key={monthKey} className="flex flex-col gap-1">
                                            ...
                                            >
                                                ...
                                                <p className={`text-sm font-medium ${total > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'}`}>
                                                    {total > 0 ? fmt(total, cardCurrency) : 'Sem gastos'}
                                                </p>
                                                {transactions.length > 0 && (
                                                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                                                        {transactions.length} {transactions.length === 1 ? 'lançamento' : 'lançamentos'} {isExpanded ? '▲' : '▼'}
                                                    </p>
                                                )}
                                            </button>
                                            {isExpanded && transactions.length > 0 && (
                                                <div className="border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden">
                                                    {transactions.map(t => (
                                                        <div key={t.id} className="flex items-center justify-between px-3 py-2 border-b border-gray-50 dark:border-gray-700 last:border-0">
                                                            <p className="text-xs text-gray-700 dark:text-gray-300 truncate max-w-[120px]">{t.description || '—'}</p>
                                                            <p className="text-xs font-medium text-red-500 flex-shrink-0 ml-2">{fmt(t.amount, cardCurrency)}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
